export function Weather({ weather }) {
  let now =
    new Date(weather.date).toString().substring(0, 3) +
    new Date(weather.date).toString().substring(7, 10);
  // Плохие названия переменных, что означает n в конце?
  let nown =
    new Date(weather.daten).toString().substring(0, 3) +
    new Date(weather.daten).toString().substring(7, 10);
  let nownn =
    new Date(weather.datenn).toString().substring(0, 3) +
    new Date(weather.datenn).toString().substring(7, 10);
  return (
